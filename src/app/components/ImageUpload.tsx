...
const ImageUpload: React.FC<ImageUploadProps> = ({
  ...
  return (
    <div className="grid grid-cols-3 gap-14 mt-2">
      {imageKeys.map((key, index) => (
        <div key={key} className="flex flex-col items-center">
          <div className="relative flex items-center">
            {/* วงกลมเลข 1, 2, 3 - มีระยะห่างจากรูป */}
            <span className="absolute left-[-40px] top-1/2 transform -translate-y-1/2 bg-gray-300 text-gray-700 w-8 h-8 flex items-center justify-center rounded-full text-lg font-bold">
              {index + 1}
            </span>
